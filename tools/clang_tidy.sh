...
if [ ! -f "${compile_commands}" ]; then
    echo "Could not find a 'compile_commands.json'. Please compile the project first."
    exit 1
fi

cxx_files_list="$(cd "${repo_root}" && git ls-tree --full-tree --name-only -r HEAD | grep -E ".*(\.cpp|\.hpp)")"

(cd "${repo_root}" && clang-tidy --warnings-as-errors --use-color $cxx_files_list -p "${build_root}")
